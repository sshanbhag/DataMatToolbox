...
classdef Data < handle
	...
	methods
		...
		%------------------------------------------------------------------------
			...
			% set file name info
			obj.fullfname = D.SourceFile;
			[obj.fpath, obj.fname, obj.fext] = filenamesplit(obj.fullfname);
		...
		%------------------------------------------------------------------------
			...
			% parse probes to fill in Nunits information
			obj.Nunits = zeros(obj.Nprobes, 1);
			for n = 1:obj.Nprobes
				obj.Nunits(n) = obj.Probes(n).nclusters;
			end
			
			% clear Segment struct
			clear Segment
			
			% return unitId if requested
			if nargout
				varargout{1} = unitID;
			end
