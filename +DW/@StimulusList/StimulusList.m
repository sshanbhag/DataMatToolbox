...
classdef StimulusList < handle
	...
	methods		
		...
		%---------------------------------------------------------------------
		%---------------------------------------------------------------------
		function C = getChannelAsNum(obj, varargin)
			DW.DataWaveDefaults;
			C = zeros(size(obj.Channel));		
			if any(obj.Channel == 'R')
				C(obj.Channel == 'R') = R * (obj.Channel == 'R');
			end
			if any(obj.Channel == 'L')
				C(obj.Channel == 'L') = L * (obj.Channel == 'L');
			end
		end
		%---------------------------------------------------------------------
		%---------------------------------------------------------------------

		function s = sweepsList(obj)
			s = unique(obj.Nsweeps);
		end
