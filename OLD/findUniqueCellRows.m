function [uniqueRows, uniqueIndices, NuniqueRows] = findUniqueCellRows(cell_to_search)
%-----------------------------------------------------------------------------
% [uniqueRows, uniqueIndices, NuniqueRows] = findUniqueCellRows(cell_to_search)
%-----------------------------------------------------------------------------
%	algorithm: 
...
% 	- adapted algorithm from findUniqueText 
%
% Revisions:
%------------------------------------------------------------------------
% TO DO:
...
% get size of cell_to_search
[Nmarkers, Ncols] = size(cell_to_search);
NuniqueRows = 0;
uniqueRows = {};
uniqueIndices = {};
textIndex = 1;
testIndex = 1;
runFlag = 1;
searchrow = cell(1, Ncols);

% Make a copy of the cell_to_search cell array, stored in testvals to search
testvals = cell_to_search;

%------------------------------------------------------------------------
%------------------------------------------------------------------------

% loop through markers or until runFlag is set to 0 by check within the loop
while runFlag && (Nunique <= Nmarkers)
	% grab new string value to search for
	for c = 1:Ncols
		searchrow{1, c} = testvals{testIndex, c};
	end
	
	% look for matches in original strings_to_search array
	matchindices = strcmp(searchrow, strings_to_search);
